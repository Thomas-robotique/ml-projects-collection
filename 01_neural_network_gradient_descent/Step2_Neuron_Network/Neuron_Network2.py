...
def gradiant(y, X, parametre, activation):
...
def uptade(a,gradiant,parametre ):
...
def plot_decision_boundary(ax, params):
...
parametre, history = Neuron_network(X, y)


fig, ax = plt.subplots() ## On créer une figure et un axe pour dessiner 

   
def animate(i):
    ax.clear()
    ax.scatter(X[0,:], X[1,:], c=y.flatten(), cmap='summer')
    plot_decision_boundary(ax, history[i])
